...
def test_strict_default_run_matches_acceptance_baseline(tmp_path: Path) -> None:
    ...
    assert _read_workbook_structure(outputs["output_workbook"]) == _read_workbook_structure(EXPECTED_DIR / "估值表解析_output_2025-03-27.xlsx")
...
def _read_workbook_structure(path: Path) -> dict[str, list[str | None]]:
    workbook = load_workbook(path, read_only=True, data_only=True)
    try:
        return {
            sheet_name: [cell for cell in next(workbook[sheet_name].iter_rows(min_row=1, max_row=1, values_only=True))]

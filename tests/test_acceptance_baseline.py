...
def test_strict_default_run_matches_acceptance_baseline(tmp_path: Path) -> None:
    ...
    assert _normalized_text(outputs["routing_results"], encoding="utf-8-sig") == _normalized_text(
        EXPECTED_DIR / "routing_results.csv",
        encoding="utf-8-sig",
    )
    ...
    assert _read_workbook_rows(outputs["output_workbook"]) == _read_workbook_rows(
        EXPECTED_DIR / "估值表解析_output_2025-03-27.xlsx"
    )
